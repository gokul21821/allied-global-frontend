...
const CallHistory = () => {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      // We need to fetch full details for each conversation to get summary and variables
      const fullDetails: any[] = [];
      const batchSize = 10;

      const conversationsToFetch = filteredConversations;

      for (let i = 0; i < conversationsToFetch.length; i += batchSize) {
        const batch = conversationsToFetch.slice(i, i + batchSize);
        const batchPromises = batch.map(async (conv) => {
          try {
            const response = await fetch(
              `${BACKEND_URL}/get-conversation/${conv.conversation_id}`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (response.ok) {
              const data = await response.json();
              return { ...conv, details: data.conversation };
            }
          } catch (e) {
            console.error(
      ...
      // Collect all unique variable names from data collection results
      const variableNames = new Set<string>();
      fullDetails.forEach((item) => {
        const results = item.details?.analysis?.data_collection_results || {};
        Object.keys(results).forEach((name) => variableNames.add(name));
      });
      const sortedVarNames = Array.from(variableNames).sort();
      ...
      fullDetails.forEach((item) => {
        ...
        const sentiment =
          conv.call_successful === "success"
            ...
            : conv.call_successful === "failed"
              ? "Negative"
              : "Unknown";
        const summary = details?.analysis?.transcript_summary || "N/A";
        const recordingLink = `${BACKEND_URL}/get-audio/${conv.conversation_id}`;

        const row = [
          `"${date}"`,
          `"${conv.agent_name.replace(/"/g, '""')}"`,
          `"${conv.agent_id}"`,
          `"${conv.conversation_id}"`,
          conv.call_duration_secs.toString(),
        ...
        sortedVarNames.forEach((name) => {
          const val = dataResults[name]?.value;
          row.push(
      ...
      link.setAttribute(
        "download",
        `call-history-detailed-${new Date().toISOString().split("T")[0]}.csv`,
      );
    ...
    .filter((conversation) => {
      ...
      const matchesDateAfter =
        ...
        conversation.start_time_unix_secs >=
          new Date(dateAfter).getTime() / 1000;

      // Date before filter
      const matchesDateBefore =
        !dateBefore ||
        conversation.start_time_unix_secs <=
          new Date(dateBefore).getTime() / 1000;

      // Agent filter
      const matchesAgent =
        !selectedAgent || conversation.agent_id === selectedAgent;

      // Evaluation filter
      const matchesEvaluation =
        !selectedEvaluation ||
        conversation.call_successful === selectedEvaluation;
            ...
            <div className="flex items-center space-x-2 flex-wrap">
              ...
              {selectedAgent && (
                <span className="inline-flex items-center space-x-1 px-2 py-1 text-xs bg-primary-50 text-primary-700 dark:bg-primary-400/10 dark:text-primary-400 rounded-full">
                  <span>
                    {
                      getUniqueAgents().find((a) => a.id === selectedAgent)
                        ?.name
                    }
                  </span>
                  <button
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      >
                        ...
                        <p className="text-2xl font-heading font-bold text-primary dark:text-primary-400">
                          {conversationDetails.conversation.metadata.cost !==
                          undefined
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="p-4 bg-gray-50 dark:bg-dark-100 rounded-xl border border-gray-200 dark:border-dark-100">
                            <p className="text-sm text-gray-500 dark:text-gray-400 italic">
                              No data collection results available for this
                              conversation.
                            </p>
                          </div>
                        </div>
