...
const UserManagement = () => {
  ...
  const [adminUserRole, setAdminUserRole] = useState<'user' | 'sub-admin' | 'admin'>('user');
                ...
                {hasAdminCreationPrivileges && (
                  <form onSubmit={handleCreateAdminUser} className="flex flex-col space-y-2">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Create regular users, admins, or sub-admins.
                    </p>
                    <input
                      type="email"
                    ...
                    <select
                      value={adminUserRole}
                      onChange={(e) => setAdminUserRole(e.target.value as 'user' | 'sub-admin' | 'admin')}
              ...
              {filteredUsers.map((userItem, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      {/* Send Request Button (for regular users only, not admins/super-admins) */}
                      {user && canSendRequest(userItem.id) && userData?.role !== 'super-admin' && userData?.role !== 'admin' && (!hasSubAdminPrivileges() || isManagedUser(userItem.id)) && (
                        <button
                      ...
                      {/* Switch Workspace Button (for sub-admins and their managed users) */}
                      {hasSubAdminPrivileges() && isManagedUser(userItem.id) && (
                        <button
                      ...
                      {/* User Settings Button (Super Admin, Admin, or Sub-Admin/Sub-Admin-User for managed users) */}
                      {(((userData?.role === 'super-admin' || userData?.role === 'admin') && userItem.id !== user?.uid) ||
                        (hasSubAdminPrivileges() && isManagedUser(userItem.id))) && (
                          <button
